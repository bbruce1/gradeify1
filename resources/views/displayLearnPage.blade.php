...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		flashArray = <?php print_r(json_encode($familiarFlashCards)); ?>;
		$(".familiarFlashCards").html(flashArray.length);
		var allArray = <?php print_r(json_encode($allFlashCards)); ?>;
		...
		$(document).on("click",'#startButton',function(){
			...
			if(newArray.length > 0){
				...
				var rArray = <?php echo json_encode($allFlashCards); ?>;
				...
				for(let i = 0; i < 4; i++){
					if(finalArray[i]){
						option += "<div class='col-6'><div class='radioCSS'><label>"+(i+1)+"</label><input type='radio' name='options' class='options options_"+ (i+1) +"' id='"+ finalArray[i]['id'] +"' > <label for='"+ finalArray[i]['id'] +"' >"+ finalArray[i]['flashcard_defination'] +"</label></div></div> ";
					}
				}

				option += '</div>';

				$(".flashTerm").html(checkNewArray[0]['flashcard_terms']);
				...
				swal({
					title: "Completed!!", 
					text: "You have already learnt this Deck", 
					type: "success",
		...
		$(document).on("change","input[type=radio][name=options]",function() {
			...
			if(checkFlashTerm == checkFlashDes){
				...
				$(this).prev('label').html('<i class="fa fa-check text-center text-light" style="position: absolute;right: -8px;top: 4px;"></i>');

				flashArray.push(checkNewArray[0]);

				$.ajax({
					url: '/saveFlashcardResult?id='+checkFlashTerm+'&type=familiar&deckId='+deckId,
					type: 'get',
					dataType: 'json',
		...
		function nextNewQuestion(flashArray){
			...
			if(checkNewArray.length > 0){
				...
				for(let i = 0; i < 4; i++){
					if(finalArray[i]){
						option += "<div class='col-6'><div class='radioCSS'><label>"+(i+1)+"</label><input type='radio' name='options' class='options' id='"+ finalArray[i]['id'] +"' > <label for='"+ finalArray[i]['id'] +"' >"+ finalArray[i]['flashcard_defination'] +"</label></div></div> ";
					}
				}
		...
		function famFlashInit(flashArray) {
			...
			$(".flashTerm").html(checkFamiliarArray[0]['flashcard_terms']);
			$(".flashTerm").attr('data-value',checkFamiliarArray[0]['flashcard_defination']);
		...
		$(document).on("click","#familiarNext",function(){
			...
			if(checkFlashFamiliarId == checkFlashFamiliarDes){
				...
				$.ajax({
					url: '/saveFlashcardResult?id='+checkFamiliarId+'&type=mastered&deckId='+deckId,
					type: 'get',
					dataType: 'json',
					success: function(data){
						console.log("done");
				...
				setTimeout(function(){ nextFamiliarQuestion(allFamFlashCards); btn.attr('disabled', false);  },2000)
